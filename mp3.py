...
def main():
    ...
    cover_art_path = f'thumbnail_{video_id}.png' if is_autogenerated else f'frame_{video_id}.png'
    if is_autogenerated:
    ...
    with open(f'{video_id}.info.json') as metadata_json_file:
        ...
        title = metadata['track'] or metadata['title']
        if artist:
            subprocess.run(['eyeD3', '--artist', f'{artist}', f'{audio_path}'], check=True)
        if album:
            subprocess.run(['eyeD3', '--album', f'{album}', f'{audio_path}'], check=True)
        if title:
